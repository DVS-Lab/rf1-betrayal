...
model="3"
sm="5"
mask=${basedir}/masks/seed-dACC.nii.gz

# Output directory for the meants text files
outputdir=${basedir}/derivatives/extractions/dACC_ugr_social_meants
log=${basedir}/code/dACC_ugr_social_meants_log.txt
mkdir -p $outputdir
...
for sub in `cat ${basedir}/code/sublist_n132.txt`; do
  ...
  DATA=${basedir}/derivatives/fsl/sub-${sub}/ses-01/L2_task-${task}_model-${model}_type-act_sm-${sm}.gfeat/cope12.feat/stats/zstat1.nii.gz

  DATArun1=${basedir}/derivatives/fsl/sub-${sub}/ses-01/L1_task-${task}_model-${model}_type-act_run-1_sm-${sm}.feat/stats/zstat12.nii.gz
  
  DATArun2=${basedir}/derivatives/fsl/sub-${sub}/ses-01/L1_task-${task}_model-${model}_type-act_run-2_sm-${sm}.feat/stats/zstat12.nii.gz
 

  # Check if the data file exists
  if [ -f "$DATA" ]; then   
     echo "Extracting L2 dACC signal for subject: ${sub}" >> $log
    OUTPUT="${outputdir}/sub-${sub}_L2_dACC_social_meants.txt" 
   ...
   if [ -f "$DATArun1" ]; then
   	 ...
   	 OUTPUT="${outputdir}/sub-${sub}_run-1_dACC_social_meants.txt"
 	...
 	if [ -f "$DATArun2" ]; then
   	 ...
   	 OUTPUT="${outputdir}/sub-${sub}_run-2_dACC_social_meants.txt"
