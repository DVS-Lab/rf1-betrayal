...
outputdir=${basedir}/derivatives/extractions/mpfc_cat${contrast}_ugr_meants
log=${basedir}/code/mpfc_cat${contrast}_ugr_meants_log.txt
mkdir -p $outputdir
...
for sub in `cat ${basedir}/code/sublist_n132.txt`; do
  ...
  DATA=${basedir}/derivatives/fsl/sub-${sub}/ses-01/L2_task-${task}_model-${model}_type-act_sm-${sm}.gfeat/cope${contrast}.feat/stats/zstat1.nii.gz

  DATArun1=${basedir}/derivatives/fsl/sub-${sub}/ses-01/L1_task-${task}_model-${model}_type-act_run-1_sm-${sm}.feat/stats/zstat${contrast}.nii.gz
  
  DATArun2=${basedir}/derivatives/fsl/sub-${sub}/ses-01/L1_task-${task}_model-${model}_type-act_run-2_sm-${sm}.feat/stats/zstat${contrast}.nii.gz
