...
#		1) activation
#		2) seed-based ppi
# Note that activation analysis must be performed first. (DVS NOTE: is this still true?)
# Seed-based PPI should follow activation analyses.

# ensure paths are correct irrespective from where user runs the script
scriptdir="$( cd "$( dirname "${BASH_SOURCE[0]}" )" >/dev/null 2>&1 && pwd )"
maindir="$(dirname "$scriptdir")"
rf1datadir=/ZPOOL/data/projects/rf1-sra-linux2 #need to fix this upon release (no hard coding paths, and should pull from OpenNeuro)
