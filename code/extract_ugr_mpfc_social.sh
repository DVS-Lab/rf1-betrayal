...
outputdir=${basedir}/derivatives/extractions/mpfc_ugr_social_meants
log=${basedir}/code/mpfc_ugr_social_meants_log.txt
mkdir -p $outputdir
...
for sub in `cat ${basedir}/code/sublist_n132.txt`; do
  ...
  if [ -f "$DATA" ]; then   
    ...
    OUTPUT="${outputdir}/sub-${sub}_L2_mpfc_social_meants.txt" 
   ...
   if [ -f "$DATArun1" ]; then
   	 ...
   	 OUTPUT="${outputdir}/sub-${sub}_run-1_mpfc_social_meants.txt"
 	...
 	if [ -f "$DATArun2" ]; then
   	 ...
   	 OUTPUT="${outputdir}/sub-${sub}_run-2_mpfc_social_meants.txt"
