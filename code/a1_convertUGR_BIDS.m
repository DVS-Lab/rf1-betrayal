...
basedir = '/Users/tur61139/Documents/GitHub/rf1-norms'; 
% basedir = '/ZPOOL/data/projects/rf1-norms'; % please absolve me for hardcoding
datadir = '/Users/tur61139/Documents/GitHub/rf1-sra/stimuli/Scan-Lets_Make_A_Deal';  
% datadir='/ZPOOL/data/projects/rf1-sra/stimuli/Scan-Lets_Make_A_Deal'
all_subjects_file = '/Users/tur61139/Documents/GitHub/rf1-sra-data/code/sublist_all.txt';
% all_subjects_file = '/ZPOOL/data/projects/rf1-sra-data/code/sublist_all.txt';

% load subject list and loop through subs
subject_files = dir(fullfile(datadir, 'logs', '1*'));% For all subjects that have a .csv file created (so presumably have MR data)
subjects = {subject_files.name};  % Extract the folder names as strings
length(subjects)
...
for s = 1:length(subjects)
    for r = 0:1
        ...
        % load raw data
        indata = fullfile(datadir,'logs',subjects{s},sprintf('sub-%05d_task-ultimatum_run-%01d_raw.csv',str2double(subjects{s}),r));
        ...
        % Create output file
        fname = sprintf('sub-%05d_task-ugr_run-%01d_events.tsv',str2double(subjects{s}),r+1); % making compatible with BIDS output
        output = fullfile(basedir,'bids',['sub-' subjects{s}],'func');
        if ~exist(output,'dir')
            mkdir(output)
        end
